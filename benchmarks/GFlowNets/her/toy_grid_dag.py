...
def get_seq(s):
    ...
    base_seq = token_string_from_tensor(ax, alphabet, from_logits=False)
...
def get_func(arg):
    if arg.func == "corner":
        ...
        def func(x):
            ax = torch.from_numpy(x).unsqueeze(0).long()
            # one hot 
          
            ax = torch.nn.functional.one_hot(ax, arg.horizon).float()
           
            r = classifier_model.model(ax).item()
            r = 2 ** r - 1
...
class GridEnv:
    ...
    def s2mode(self, s):
        x = np.asarray(s)
        reward = log2(self.func(self.s2x(x)) + 1)
        if reward > 0.96:
            return True
    ...
    for i in tqdm(range(args.n_train_steps + 1), disable=not args.progress):
        ...
        if i % eval_every == 0 or i == args.n_train_steps:
            ...
            if args.progress:
                ...
                rewards = np.sort(
                    [ log2(env.func(env.s2x(np.asarray(s))) + 1)  for s in all_visited[-recent:]]
                )  # ascending
