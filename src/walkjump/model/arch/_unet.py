...
class UNetArch(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self._tconvs = torch.nn.ModuleList([
            ...
            torch.nn.Sequential(
                ...
                torch.nn.LogSigmoid(),
                torch.nn.Conv2d(chs[0], 2, kernel_size=3, padding=1),  # (batch, 1, 28, 28)
            ),
        ])

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (..., 28 * 28, (alphabet_size), ) for discrete mnist alphabet size is 2 (0 and 1)
        x = torch.movedim(x, -2, -1) # (..., ch0, 28 * 28)
        x2 = torch.reshape(x, (*x.shape[:-1], 28, 28))  # (..., ch0, 28, 28)
        ...
        for i, tconv in enumerate(self._tconvs):
            if i == 0:
                ...
                signal = tconv(signal)
        signal = torch.reshape(signal, (*signal.shape[:-3], -1, 2))  # (..., 28 * 28, 2)
        return signal
