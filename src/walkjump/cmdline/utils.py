...
def instantiate_model_for_sample_mode(
    ...
    if isinstance(model, DenoiseModel) and sample_mode_model_cfg.guide_path is not None:
        print(
            "[instantiate_model_for_sample_mode] (model.guide_model)",
            _LOG_MSG_INSTANTIATE_MODEL.format(
                model_type="guide", checkpoint_path=sample_mode_model_cfg.guide_path
            ),
        )
    ...
    if isinstance(model, DenoiseModel) and sample_mode_model_cfg.guide_path2 is not None:
        print(
            "[instantiate_model_for_sample_mode] (model.guide_model2)",
            _LOG_MSG_INSTANTIATE_MODEL.format(
                model_type="guide", checkpoint_path=sample_mode_model_cfg.guide_path2
            ),
        )
        model.guide_model2 = AbDiscriminatorModel.load_from_checkpoint(sample_mode_model_cfg.guide_path2)
        model.guide_model2.eval()
        model.guide_model2.training = False
